...
if %ERRORLEVEL% EQU 0 (
    ...
    if %ERRORLEVEL% EQU 0 (
        ...
        echo %YELLOW%⚠ 无法连接到Python API服务%RESET%
        echo %YELLOW%请在另一个终端窗口运行 'start-python-service.bat'%RESET%
    ...
    echo %YELLOW%⚠ 未找到curl，无法检查Python API服务%RESET%
    echo %YELLOW%请确保Python API服务在另一个终端窗口中运行%RESET%
)
